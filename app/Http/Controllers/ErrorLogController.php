...
class ErrorLogController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->has('errorMessage')) {
            $query->where('errorMessage', $request->input('errorMessage'));
        }
        if ($request->has('message')) {
            $query->where('message', $request->input('message'));
        }
        if ($request->has('stack')) {
            $query->where('stack', $request->input('stack'));
        }
        if ($request->has('details')) {
            $query->where('details', $request->input('details'));
        }
    ...
    public function store(CreateErrorLogRequest $request): JsonResponse
    {
        ...
        $data = ErrorLog::create($request->all());
    ...
    public function update(CreateErrorLogRequest $request, $id): JsonResponse
    {
    ...
    public function getSchema(): JsonResponse
    {
        return response()->json([
            \Illuminate\Support\Facades\DB::select("DESCRIBE error_logs")
        ]);
    }
}
