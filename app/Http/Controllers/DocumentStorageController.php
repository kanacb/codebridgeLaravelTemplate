...
if ($request->has('size')) {$query->where('size', $request->input('size'));}
if ($request->has('path')) {$query->where('path', $request->input('path'));}
if ($request->has('lastModifiedDate')) {$query->where('lastModifiedDate', $request->input('lastModifiedDate'));}
if ($request->has('lastModified')) {$query->where('lastModified', $request->input('lastModified'));}
if ($request->has('eTag')) {$query->where('eTag', $request->input('eTag'));}
if ($request->has('url')) {$query->where('url', $request->input('url'));}
if ($request->has('tableId')) {$query->where('tableId', $request->input('tableId'));}
if ($request->has('tableName')) {$query->where('tableName', $request->input('tableName'));}
    ...
    public function store(CreateDocumentStorageRequest $request): JsonResponse 
    {
        ...
        //     ]);
        // }
        $data = DocumentStorage::create($request->validated());
    ...
    public function getSchema() : JsonResponse{
        return response()->json([
            \Illuminate\Support\Facades\DB::select("DESCRIBE documentStorages")
        ]);
    }
